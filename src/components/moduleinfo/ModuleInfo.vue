<template src = "./modInfo.html"> </template>
<style src = "./modInfo.css"> </style>
...
/*      
      ...
      for (let j = 0; j < matches.length; j++) {
        let button = matches[j];
        button.addEventListener('click', function() {vm.modInfo(arr[j][1]);});
}  */
 ...
 //PRECLUSIONS
      ...
      var arr_a = [];      
      for(i=0; i <this.mods.length; i++){
        var index = mod.preclusion.toUpperCase().indexOf(this.mods[i].code);
       if(index != -1){
      ...
      var arr_all = arr.concat(arr_a);
      
      //onclick of the module code, brings you to the module page      
      var matches = document.querySelectorAll("a");
      for (let j = 0; j < matches.length; j++) {
        let button = matches[j];
        button.addEventListener('click', function() {vm.modInfo(arr_all[j][1]);});
}  
